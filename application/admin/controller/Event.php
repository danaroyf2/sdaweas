...
class Event extends Controller
{
    ...
    public function getswitch()
    {
        ...
        $admin = Admins::table('wolive_service')->where(['service_id'=>$post['service_id'],'business_id'=>$post['business_id']])->find();
        if(empty($admin)){
        ...
        Admins::table('wolive_queue')->where("visiter_id", $post['visiter_id'])->where('business_id', $post['business_id'])->where('state', 'normal')->update(['service_id' => $post['service_id']]);
        $arr = ['code' => 0, 'msg' => '转接成功！'];
        return $arr;
    }
            ...
            foreach ($payload['events'] as $event) {
                ...
                if ($event['name'] == 'channel_removed') {
                    ...
                    if (strpos($event['channel'], 'kefu') === 0) {
                        ...
                        $outtime1=date("Y-m-d H:i:s",time());
                        $res2 = Admins::table("wolive_visiter")->where("channel", $channel)->update(["state" => 'offline','outtime'=>$outtime1]);
                    }
                }
                ...
                if ($event["name"] == "channel_added") {
                    ...
                    if (strpos($event['channel'], 'kefu') === 0) {
                        ...
                        $res2 = Admins::table("wolive_visiter")->where("channel", $channel)
                        ->update(["state" => 'online','logintime'=>$thistime,'outtime'=>$outtime]);
                    }
                }
            }
...
//            if (empty($sended) && $service_data['state']=='offline') {
            if ($service_data['state']=='offline') {
                TplService::send($arr["business_id"],$service_data['open_id'],url('weixin/login/callback',['business_id'=>$arr['business_id'],'service_id'=>$service_id],true,true),$wechat['msg_tpl'],[
                    "first"  => "你有一条新的信息!",
                    "keyword1"   => $visiter["visiter_name"] ?$visiter["visiter_name"]:'游111客'.$arr['visiter_id'],
...
//                Admins::table('wolive_queue')->where('business_id', $arr['business_id'])->where('visiter_id', $arr['visiter_id'])->update(['remind_tpl'=>1]);
    ...
    public function notice()
    {
        ...
        $arr['extends'] = json_encode(['browserName'    => $ua->getBrowserNameCn(),
            ...
            //'os'             => $ua->getOperatingSystemCn().$ua->getBrowserVersion().'-'.$ua->getBrowserNameCn(),
            'os'=>$_SERVER['HTTP_USER_AGENT'],
            'engine'         => $ua->getEngine(),
            'getFullName'         => $ua->getFullName()
        ...
        //判断是否有数据如果没有数据添加打招呼内容
        //halt($arr);
        //$service = Admins::table('wolive_queue')->where(['visiter_id' => $visiter_id, 'business_id' => $business_id])->where('state', 'normal')->find();
        //$service_id = $service['service_id'];
        $service_data = Admins::table("wolive_service")->where('business_id', $arr["business_id"])->find();
        $findchats=db("wolive_chats")->where(['visiter_id'=>$visiter_id,'service_id'=>$service_data['service_id']])->find();
        //dump(['visiter_id'=>$arr['visiter_id'],'service_id'=>$arr['service_id']]);
        //dump($findchats);
        //halt($findchats);
        if(empty($findchats)){
            ...
            $thistime=time();
            $dazhaohu_data= db("wolive_sentence")->where(['service_id'=>$service_id,'state'=>'using'])->order('weigh')->select();
            //wolive_chats
            //dump('dazhaohu_data');
            //dump(['service_id'=>$service_id,'state'=>'using']);
            //dump($dazhaohu_data);
            $dazhaohulist=[];
            foreach ($dazhaohu_data as $dazhaohu){
                $dazhaohulist[]=[
                    'visiter_id'=>$visiter_id,
                    'service_id'=>$service_id,
                    'business_id'=>$business_id,
                    'content'=>$dazhaohu['content'],
                    'timestamp'=>$thistime,
                    'state'=>'readed',
                    'direction'=>'to_visiter',
                ];
            }
            db("wolive_chats")->insertAll($dazhaohulist);
        };
        //
        if ($business['distribution_rule'] == 'claim') {
            ...
            //$visiter = Admins::table('wolive_visiter')->where(['business_id' => $business_id])->find();
            if ($visiter) {
                
                if(empty($visiter['avatar'])){
                    Admins::table('wolive_visiter')->where(['business_id'=>$visiter['business_id']])->update([
                        'avatar'=>$_POST['avatar'],
                        'visiter_name'=>$_POST['visiter_name'],
                    ]);
                }
                ...
                if ($service_data) {
                    ...
                    if ($state == 'online') {
                        ...
                        $service_data['content_type'] = $words['type'];
            ...
            $visiter = Admins::table('wolive_visiter')->where(['visiter_id' => $visiter_id, 'business_id' => $business_id])->find();
            //增加登录次数
            Admins::table('wolive_visiter')->where(['visiter_id'=>$visiter_id])->update(['login_times'=>($visiter['login_times']+1)]);    
            //$visiter=false;
            if ($visiter) {
                //真正的老用户
                $findvisiter=db('wolive_visiter')->where(['visiter_id'=>$visiter_id])->find();
                if(empty($findvisiter['ipuseragent'])){
                    $ip = $_SERVER['REMOTE_ADDR'];
                    $userAgent = $ip.$_SERVER['HTTP_USER_AGENT'];
                    db('wolive_visiter')->where(['visiter_id'=>$visiter_id])->update(['ipuseragent'=>$userAgent]);
                }
                ...
                if ($service['state'] == 'in_black_list') {
                    $service_data = Admins::table("wolive_service")->field('avatar,business_id,email,groupid,nick_name,service_id,state')->where('service_id', $service_id)->find();
                    $service_data['content'] = "您好！";
                    $returndata = ['code' => 0, 'msg' => 'success', 'data' => $service_data];
                    return $returndata;
                }
                ...
                if ($service_data) {
                    ...
                    $wordslis = Admins::table('wolive_sentence')->where("service_id", $service_data['service_id'])->where('state', 'using')
                    ->order('weigh')
                    ->select();
                    
                    $dazhaohulis=[];
                    if (!empty($wordslis)) {
                        foreach ($wordslis as $w){
                            $dazhahu=serialize($service_data);
                            $dazhahu=unserialize($dazhahu);
                            $dazhahu['content'] = htmlspecialchars_decode($w['content']);
                            $dazhahu['content_type']=$w['content_type'];
                            $dazhaohulis[]=$dazhahu;
                        }
    
                    } else {
                        $service_data['content'] = "您好！";
                        $dazhaohulis[]=$service_data;
                    }
                    
                    $service_data['content_type'] = $words['type'];
                    $service_data['content_type'] = "text";
                    unset($service_data['open_id']);
                    
                    
                    //---------------
                    $isnew=true;
                    if($visiter['login_times']>=1){
                        $isnew=false;
                    }
                    $token=md5(time());
                    $returndata = ['code' => 0, 'msg' => 'success', 'data' =>$dazhaohulis,'isnew'=>$isnew,'token'=>$token];
                ...
                $pusher->trigger("ud" . $service_data['service_id'], 'on_notice', array('message' => $arr));
                 ...
                 $words = Admins::table('wolive_sentence')->where("service_id", $service_data['service_id'])->where('state', 'using')->find();
                $wordslis = Admins::table('wolive_sentence')->where("service_id", $service_data['service_id'])
                    ->where(['state'=>'using'])
                    //->order('weigh desc')
                    ->order('weigh')
                    ->select();
                ...
                //$returndata = ['code' => 0, 'msg' => 'success', 'data' => $service_data];
                $returndata = ['code' => 0, 'msg' => 'success', 'data' =>$dazhaohulis];
...
//                控制频繁度客服离线才发送 微信：zrwx978
                if($service_data['open_id'] && $service_data['state']=='offline'){
                    try {
                        TplService::send($arr["business_id"],$service_data['open_id'],url('weixin/login/callback',['business_id'=>$arr['business_id'],'service_id'=>$service_data['service_id']],true,true),$wechat['visitor_tpl'],[
                            "first"  => "您有新访客！",
                            "keyword1"   => $arr["visiter_name"] ?$arr["visiter_name"]:'游22客'.$arr['visiter_id'],
                            "keyword2"  => date('Y-m-d H:i:s',time()),
    ...
    public function chatdata()
    {
        ...
        if ($post["hid"] == '') {
            ...
            $data = Admins::table('wolive_chats')->where(['service_id' => $service_id, 'visiter_id' => $vid, 'business_id' => $post['business_id']])->order('cid desc')
            //->limit(20)
            ->select();
            ...
            $data = Admins::table('wolive_chats')->where(['service_id' => $service_id, 'visiter_id' => $vid, 'business_id' => $post['business_id']])->where('cid', '<', $post['hid'])->order('cid desc')->limit(10)->select();
            $vdata = Admins::table('wolive_visiter')->where('visiter_id', $vid)->find();
            $sdata = Admins::table('wolive_service')->where('service_id', $service_id)->find();
        ...
        //修改读取状态
        Admins::table('wolive_chats')
        ->where('visiter_id',  $vid)
        ->where('business_id', $post['business_id'])
        ->where(['direction'=>'to_visiter'])
        ->update(['state' => 1]);
        //
    ...
    public function getquestion()
    {
        ...
        if ($result) {
            ...
            $arr = ['code' => 1, 'msg' => 'error', 'data' => '暂无数据'];
            return $arr;
        }
    }
    ...
    public function getanswer()
    {
        ...
        //检查是否拉黑
        $statequeue=Admins::table('wolive_queue')
                    ->where(['business_id'=>$result['business_id'],'visiter_id' =>$visiter_id])
                    ->value('state');
        if($statequeue=='in_black_list'){
