...
echo $date;
        $sql="select  * from jiaolian  order by Id desc";		
        //echo $sql;		
        ...
        while($row=mysqli_fetch_assoc($rs))
        {
            ...
            <tr class="text-c">
                ...
                <td ><?php echo $row["shanchang"]?></td>
                <td >
                <img src="uploads/<?php echo $row["imgs"]?>"  width="100px" height="100px"/>
                </td>
              <td ><a href="jiaoliantouxiang.php?id=<?php echo $row["id"]?>"><div id="yangshi">修改头像</div></a></td>
               <td >
                  <?php
                  if($row["ipone"]==0)
                  {
                      ?>
                      <a style="text-decoration:none; color: #FFFFFF ;" onClick="lizhi(this,<?php echo $row["id"]?>)" href="javascript:;" title="入职中"> <div id="yangshi">入职中</div></a> 
                      <?php
                  }
                  else {
                      ?>
                      <a style="text-decoration:none; color: #FFFFFF ;" onClick="ruzhi(this,<?php echo $row["id"]?>)" href="javascript:;" title="已离职"> <div id="yangshi">已离职</div></a> 
                      <?php
                  }
                  ?>
...
/*同意*/
function lizhi(obj,id){
	layer.confirm('教练确定离职吗？',function(index){
		$.post("php.php",{"app":"lizhi","id":id,},function(data){
			if(data==1)
			{
...
/*驳回*/
function ruzhi(obj,id){
	layer.confirm('教练确定复职吗？',function(index){
		$.post("php.php",{"app":"ruzhi","id":id,},function(data){
			if(data==1)
			{
				$(obj).parents("tr").remove();
				layer.msg('执行成功!',{icon:1,time:1000});
