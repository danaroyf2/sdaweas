...
//        if(!isset($post['username']) || !isset($post['password']) || !isset($post['business_id'])){
    ...
    public function renew(){
     ...
     public function loginrenew(){
        ...
        $kami=isset($post['kami'])?$post['kami']:'';
        ...
        $bu = Admins::table('wolive_business')->where('id', $admin['id'])->update(['expire_time'=>strtotime($rqi)]);
        $res= Admins::table('wolive_kami')->where('id', $mi['id'])->update(['jihuo'=>date('Y-m-d H:i:s',time()),'dqtime' => $rqi,'business_id'=>$admin['id'],'ytime'=>$expire_time]);
         if($res){
