...
//                        不相等则更新
                                       db('wolive_weixin')->where(['business_id'=>$business_id,'open_id'=>$openid])->update(['subscribe' => $subscribe]);
                                   }
                               }else{
                                   db('wolive_weixin')->insert(['subscribe' => $subscribe,'business_id'=>$business_id,'open_id'=>$openid,'subscribe_time'=>0]);
                               }
                               ...
                               db('wolive_service')->where(['service_id'=>$eventKey[1]])->update(['open_id'=>$openid]);
    ...
    private function upscribe($business_id,$openid,$subscribe,$eventKey,$pusher){
       if(!$eventKey)return false;
        ...
        if(isset($eventKey[2]) && $eventKey[1]=='kefu' && $eventKey[2]){
            if($subscribe){
                //关注
                db('wolive_service')->where(['service_id'=>$eventKey[2]])->update(['open_id'=>$openid]);
                $pusher->trigger("kefu".$eventKey[2], "bind-wechat", array("message" => "绑定成功！"));
            }else{
                //取消关注
                db('wolive_service')->where(['service_id'=>$eventKey[2]])->update(['open_id'=>'']);
            }
...
//                        不相等则更新
            ...
            //通用取消关注
            db('wolive_weixin')->where(['business_id'=>$business_id,'open_id'=>$openid])->update(['subscribe' => $subscribe,'subscribe_time'=>time()]);
            db('wolive_service')->where(['business_id'=>$business_id,'open_id'=>$openid])->update(['open_id'=>'']);
        }
    }
