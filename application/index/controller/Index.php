...
class Index extends Controller
{
    ...
    public function _initialize()
    {
        ...
        //halt(wxfengjincheck('ccf3wca.store'));
        //halt($this->getsurvivalhost());
    }
    

    public function getsurvivalhost(){
        $name='';
        while (true) {  
            $yuming=db('wolive_yuming')->where(['leixing'=>'1','type'=>'0'])->orderRaw('rand()')->find();
            //halt($yuming);
            if(empty($yuming)){
                 halt('无可用域名！');
            }
            if(!wxfengjincheck($yuming['name'])){
                $name=$yuming['name'];
                //dump('可用域名:'.$yuming['name']);
                break;
            }else{
                //dump('被封域名:'.$yuming['name']);
                $jinyongtime=date('Y-m-d H:i:s',time());
                db('wolive_yuming')->where(['id'=>$yuming['id']])->update(['type'=>'1','jyshijian'=>$jinyongtime]);
            }
        }
        //die();
        return $name;
    ...
    public function checkym(){
        ...
        foreach ($yuminglist as $ym){
            ...
            if($issurvival){
                ...
                db('wolive_yuming')->where(['id'=>$ym['id']])->update(['type'=>'1','jyshijian'=>$jinyongtime]);
            }
            sleep(8);
        }
        //wxfengjincheck
    ...
    //http://k5k1g5mu.store:80/a?yzm=15922&service_id=2
    public function checkjump(){
        ...
        //halt($findservice);
        if($findservice['isqrcodeshixiao']=='1'){
             halt('二维码已失效!');
        }
        $ptym=db('wolive_yuming')->where(['name'=>$findservice['ptym'],'type'=>'0'])->find();
        //halt($ptym);
        if(empty($ptym)){
            halt('二维码已封禁!');
        }
        ...
        $checkres=checkendtime($findservice['kami']);
        if($checkres['code']!='1'){
            halt($checkres['msg']);
        }
        
        //halt( wxfengjincheck('hyxtev9.store'));
        
        /*
        if(empty($survival_host['putonghost'])){
            $survival_host['putonghost']=$this->getsurvivalhost();
            db('wolive_survival_host')->where(['service_id'=>$serverid,'erweicode'=>$yzm])->update(['putonghost'=>$survival_host['putonghost']]);
        }else{
            if(!wxfengjincheck($survival_host['putonghost'])){
                $survival_host['putonghost']=$this->getsurvivalhost();
                db('wolive_survival_host')->where(['service_id'=>$serverid,'erweicode'=>$yzm])->update(['putonghost'=>$survival_host['putonghost']]);
            }
             $yuming=db('wolive_yuming')->where(['leixing'=>'1','type'=>'0','name'=>$survival_host['putonghost']])->find();
        ...
        */
         $survival_host['putonghost']=$this->getsurvivalhost();
         $survival_host['putonghost']=getIntactHost($survival_host['putonghost']);
         $url='http://'.$survival_host['putonghost'].$findservice['qrurl'];
         //halt($url);
         header('Location:'.$url);
         exit();
     ...
     public function zscheckjump(){
        ...
        //halt($yzm);
        $findservice=db('wolive_service')->where(['service_id'=>$service_id])->find();
        if(empty($findservice)){
            halt('链接不正确或已失效!');
        }
        $findzshost=db('wolive_survival_zshost')->where(['service_id'=>$service_id,'zsercode'=>$yzm])->find();
        ...
        foreach ($yuming as $ym){
            if(!wxfengjincheck($ym['name'])){
                //halt('可用域名:'.$ym['name']);
                $url='http://'.$ym['name'].$findservice['zsqrurl'];
                //halt($url);
                header('Location:'.$url);
                exit();
            }else{
